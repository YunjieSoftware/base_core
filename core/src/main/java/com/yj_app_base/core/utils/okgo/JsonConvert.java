...
import com.yj_app_base.core.utils.GsonUtil;
    ...
    private T parseParameterizedType(Response response, ParameterizedType type) throws Exception {
        ...
        if (rawType != BaseInfo.class) {
            ...
            if (typeArgument == Void.class) {
                // 泛型格式如下： new JsonCallback<BaseInfo<Void>>(this)
                SimpleResponse simpleResponse = GsonUtil.fromJson(jsonReader, SimpleResponse.class);
                response.close();
                //noinspection unchecked
                return (T) simpleResponse.toLzyResponse();
            } else {
                // 泛型格式如下： new JsonCallback<BaseInfo<内层JavaBean>>(this)
                BaseInfo baseInfo = GsonUtil.fromJson(jsonReader, type);
                response.close();
                int code = baseInfo.code;
                //这里的0是以下意思
                ...
                if (code == 0||code==20000) {
                    ...
                    throw new IllegalStateException("错误代码：" + code + "，错误信息：" + baseInfo.msg);
                }
            }
        }
    }
